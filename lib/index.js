...
var typeStore = {
     ...
     */
    describeStore: function (storeModel) {
        ...
        this.descriptions.set(storeModel.identifier, tslib_1.__assign({}, storeModel.model));
    },
    /**
     * This method returns all of the store descriptions in the store.
     */
    getStoreDescriptions: function () {
     ...
     */
    getStoreDescription: function (identifier) {
    ...
    getState: function (identifier) {
        var currentState = this.state.get(identifier).currentState;
    ...
    dispatch: function (_a) {
        var identifier = _a.identifier, model = _a.model;
        // First we need to get the store descriptor
        var descriptor = this.getStoreDescription(identifier);
        ...
        Object.keys(model).forEach(function (key) {
            ...
            if (!descriptor[key].checker(model[key])) {
                ...
                throw new Error("Type \"" + typeof model[key] + "\" cannot be setted to the property " + key + " described as a \"" + descriptor[key].name + "\"");
            }
        });
        // If all of the keys pass the type-checking then we proceed to set it into the store
        var previousState = this.state.get(identifier); // Get the previous state of the store
        var nextState; // Declare the next state where we will set the new state
        var nextStateObject; // Declare the next state object that will be set in the new state
        ...
        if (typeof previousState !== 'undefined') {
            ...
            nextState = previousState._data.concat([nextStateObject]);
        }
        else {
            /**
        ...
        var currentState = nextState.length - 1; // Declare the current state index of the store to time travel
        this.state.set(identifier, tslib_1.__assign({}, dispatchedModel, { currentState: currentState, _data: nextState }));
     ...
     * @param {string} identifier
     */
    travelForwards: function (identifier) {
        // Get the state
        var state = this.state.get(identifier);
        // Obtain the nextStateIndex that is the current state + 1
        var nextStateIndex = state.currentState + 1;
        ...
        if (nextStateIndex >= state._data.length) {
            ...
            this.state.set(identifier, tslib_1.__assign({}, state, { currentState: nextStateIndex }));
     ...
     * @param {string} identifier
     */
    travelBackwards: function (identifier) {
        // Get the state
        var state = this.state.get(identifier);
        // Obtain the nextStateIndex that is the current state - 1
        var previousStateIndex = state.currentState - 1;
        // Check if we have and index out of bounds :)
        if (previousStateIndex < 0) {
            ...
            // Set the Current State to the next state index
            this.state.set(identifier, tslib_1.__assign({}, state, { currentState: previousStateIndex }));
            // Then call the subscribed listener of the store with the current state :)
            this.listeners.get(identifier)(this.getState(identifier));
        }
    },
     ...
     * @param {number} index
     */
    travelTo: function (identifier, index) { },
    /**
     ...
     */
    subscribe: function (store, listener) {
